...
# by semla to create our Seurat object
...
se.HER2[["Spatial3"]] <- CreateAssayObject(counts = subset(se.HER2[["Spatial"]], 
                                                           features = VariableFeatures(se.HER2))$counts)
...
We can see that the global minimum of the bottom function is achieved at x = 23, 
so the model has determined that 23 factors can describe the variability of our dataset.
...
In both matrices, we have **23** columns, 1 for each factor. In the first matrix _H_,
...
Factor 23 looks kinda interesting, almost as if it's surrounding the DCIS
...
print(sprintf("Factor 23 is associated with process %d", ans.q11))
...
[website](https://ludvigla.github.io/semla/)
