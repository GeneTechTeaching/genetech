...
output:
  html_document:
    ...
    theme: "readable"
---
<!-- Lab creator: Alma Andersson, Ludvig Larsson, Javier Escudero -->
...
A thin tissue slice, collected either from a fresh frozen or FFPE tissue block, is placed
on top of this array followed by enzymatic permeabilization to open up the cells in the tissue.
mRNA molecules can then diffuse freely from the cells and a fraction of these molecules 
...
(complementary DNA) molecule synthesized from the captured mRNAs; hence, we add the information
of which spatial location (spot) that each transcript was captured. Once the cDNA molecules are
sequenced, we can map them back to their spatial position - using the spatial barcodes -and by
doing so obtain spatial gene expression information. The Visium slide arrays have 5000 printed
spots, but the tissue section usually cover only a few thousand of these.

One *key* fact that should be emphasized is that Visium does not operate on a single cell level. 
...
At the beginning of 2024, 10X Genomics released a new version of Visium, named *VisiumHD*, which aims
to correct this very issue by changing the structure of the array. The spots printed on 
the Visium arrays have a diameter of 55um, which is the main reason multiple cells are 
captured in each spot (the average size of a eukaryote cell is around 10um in diameter). 
With VisiumHD, the printed "spots" have a size of 2um, which implies a significant increase 
in the resolution of the technology. At the same time, the whole surface of the array is 
covered in capture areas, with no empty space between them. This is different from the normal
Visium arrays, that have empty space between each spot. 
You can think of the Visium arrays as a dalmatian's fur, and the VisiumHD arrays as squared notebook.

Anyway, for this lab we will focus exclusively on the analysis of Visium data, as it is 
more straightforward than VisiumHD's.
...
fly. The code below takes care of this, all you need to do is to execute it.
...
Below we will use the `semla` R package to load all the data we need for the lab.
The data will be stored in a `Seurat` object, just like the one used in lab 3.
While the object structure is similar, there are a few extras here that will hold
the spatial information. With `semla` we also have access to a lot of very useful
functions designed to work with spatial data.
...
se.HER2 <- ReadVisiumData(infoTable)
...
`se.HER2[[]]`. So far, we have three columns named "orig.ident", "nCount_Spatial", 
"nFeature_Spatial".
...
>
> *hint* : You can use the function `LayerData()` to retrieve to expression matrix
> (i.e. UMI counts).
...
ans1 <- 3798 #NA
ans2 <- 36601 # NA
ans3 <- sum(se.HER2$nCount_Spatial)
ans4 <- "LINC01360"# NA
...
can do with the `MapLabels()` function. 

Note that requires a `column_name` value to draw on the plot. Here we use "orig.ident" 
...
MapLabels(se.HER2, column_name = "orig.ident", pt_size = 1.5)
...
Now, doing stuff with available libraries is very useful, but it is also very
important to get a feeling of what the data you are working with contains. You can
easily recreate the plot we've done above with your own custom code, simply by 
accessing the relevant information either inside of your loaded Seurat object,
or by looking at the input files directly. We need the following information:
1. The coordinates _x and y_ of the spots.
2. Something similar to the _orig.ident_ column.

```{r retrieve_spatial_coordinates}

# from the seurat object
# we can access the spatial coordinates from the seurat object by ourselves
coords.a <- se.HER2@tools[["Staffli"]]@meta_data %>% 
  select(c(barcode, x, y, sampleID))
# by using a function from semla
coords.b <- GetCoordinates(se.HER2) %>% 
  select(c(barcode, x, y, sampleID))

# from the input files
coords.c <- read_csv("./data/spatial/tissue_positions_list.csv", 
                     col_names = FALSE, col_types = "ciiiii")
colnames(coords.c) <- c("barcode", "in_tissue", "y", "x", "pxl_row_in_fullres", "pxl_col_in_fullres")
coords.c <- coords.c %>% 
  filter(in_tissue == 1) %>% 
  select(c(barcode, x, y)) %>% 
  mutate(sampleID = as.integer(1)) %>% 
  arrange(barcode)

# check that all ways to obtain the information render the same dataframe
head(coords.a)
head(coords.b)
head(coords.c)
identical(coords.a, coords.b)
identical(coords.a, coords.c)

```

Awesome! Now that we have retrieved the spatial information, we can move on to
do some spatial plotting of our own! 

```{r manual_plotting}

p1 <- ggplot(data = coords.a, aes(x = x, y = -y, color = sampleID)) +
  geom_point(size = 1) +
  theme_void() +
  theme(plot.margin = unit(c(0.05, 0.05, 0.05, 0.05), "npc")) +
  guides(color = "none")

p2 <- MapLabels(se.HER2, column_name = "orig.ident", pt_size = 1, override_plot_dims = TRUE) &
  theme(legend.title = element_blank(),
        legend.position = "none",
        plot.subtitle = element_blank())
p2 <- ModifyPatchworkTitles(p2, titles = c("Semla's plotting")) 

p <- p1 + ggtitle("Your own plotting!") | p2
p

```

Great job!! Now we are gonna have to be careful, with skills like that you might
put us out of a job.
...
> Use `MapFeatures()` to visualize the number of unique genes ("nFeature_Spatial") per
> spot. Add an argument *colors = viridis::turbo(11)* to get a more fancy color scale.

_Comment_ : While `MapLabels()` and `MapFeatures()` are different functions, they achieve the same
goal: plotting information with spatial coordinates. Their main difference is that 
`MapLabels()` is used for plotting categorical variables (like the identity of our spots), 
while `MapFeatures()` is used for discrete/continuous variables (like the total number of
unique genes per spot). `MapFeatures()` requires an argument named `feature`,
while `MapLabels()` required one named `column_name`
...
of unique genes. So why do certain regions have higher number of unique genes? As so
often in science, the answer is that it depends. You have already seen the H&E image
for the tissue section, so by now you might have a clue about why some regions have
higher numbers of genes than others. For example, certain parts of the tissue might be
harder to permeabilize than others, which would result in lower mRNA capture.
However, as a rule of thumb we expect to see higher numbers in more _cell dense_ regions.
Information about cell density is easy to get from the H&E image where cell dense regions are
...
width of 600 pixels by setting `image_height = 600`.

```{r, fig.width=6, fig.height=6}

se.HER2 <- LoadImages(se.HER2, image_height = 600)
ImagePlot(se.HER2)

```

Then, if we want to plot features on top of our H&E image(s), we will use the
`MapFeatures()` function again, but this time with an extra argument `image_use = "raw"`
...
# You can play around with the pt_size and pt_alpha (opacity) to make the background more 
# visible and also use scale_alpha = TRUE to scale the spots' transparency to the value of
# the variable to plot, further increasing background visibility. scale_alpha will only work if
# you dont have a pt_alpha set.
MapFeatures(se.HER2, 
            feature = "ERBB2", 
            image_use = "raw",
            pt_alpha = 0.7,
...
> Armed with the `MapFeatures()` function, it's now up to you to find out which
...
> -   ACTA1
>
> Use the `MapFeatures()` function inspect the spatial distribution of these genes 
...
# using the MapFeatures() function, include the HE image in the plot
...
Having access to some new tools and gained more understanding of the data, we are now
almost ready to move on to even more exciting stuff!

## Data Pre-processing

There is actually one more not-so-exciting but important thing left to do,
data pre-processing, which includes filtering and normalization.
...
histograms of the meta data `nCount_Spatial` and `nFeatures_Spatial`. The former
...
h1 <- ggplot(se.HER2[[]], aes(nCount_Spatial)) +
...
h2 <- ggplot(se.HER2[[]], aes(nFeature_Spatial)) +
...
Dashed black lines have been added at the values 2000 (`nCount_Spatial`) and 800
(`nFeature_Spatial`), these are the limits which we will settle on - these are
...
>
> 1.  `nCount_Spatial` should be larger or equal to *2000*
> 2.  `nFeature_Spatial` should be larger or equal to *800*
>
...
keep.spots <- se.HER2[[]] %>% 
  filter(nFeature_Spatial >= 800 & nCount_Spatial >= 2000) %>% 
  rownames()  #NA
...
nCount.UMI <- Matrix::rowSums(LayerData(se.HER2, assay = "Spatial", layer = "counts"))
...
nObs.genes <- Matrix::rowSums(LayerData(se.HER2) > 0)
...
keep.genes <- unique(filter(gene.data, nCounts >= 300 &
                              nObs > 5) %>% rownames())
# keep.genes <- NA
```
...
> provided in `semla` which can take a vector of spots that should be passed to the
...
ans.q7.1 <- ans2 - length(keep.genes)
ans.q7.1 <- 36601 - 12703
ans.q7.2 <- ans1 - length(keep.spots)
ans.q7.2 <- 3798 - 3745
...
couple of minutes! *verbose* is set to FALSE here but this can be removed to get 
printed information what the function is currently doing and it's progress.

```{r sctrans, echo=FALSE, eval=TRUE}

se.HER2 <- SCTransform(se.HER2, verbose = FALSE, assay = "Spatial")
se.HER2
...
**TLDR: Don't change the seed value plz, not even to 80085 or 42**.
...
real physical space_. In order to do so, we use the `MapLabels()` function to
color the spots by their cluster identity. We use `MapLabels()` because the 
cluster classification is a discrete set of values.
...
MapLabels(se.HER2, column_name = "seurat_clusters", pt_size = 1.5, pt_alpha = 0.5,
          image_use = "raw", colors = viridis::turbo(n = length(unique(se.HER2$SCT_snn_res.0.2)))) &
...
_Comment_ : For plotting gene expression, we need to use the `MapFeatures()` function 
instead of `MapLabels()`. Can you guess why?
...
#Plot first marker gene for each cluster spatially
MapFeatures(se.HER2, features = de.markers.one$gene, ncol = 3, pt_size = 0.5, pt_alpha = 0.5,  
            add_scalebar = TRUE)
...
Such factors can be _learnt from the data_, but also defined _a priori_; a good
...
profiles as latent factors. Now we will focus on factorization where no input
is required from us, but you should know that most of these algorithms also accept
user input regarding the number of factors to identify.
...
We can run the NNMF algorithm using the `RunNMF()` function. The implementation 
of NMF we are going to use is tremendously fast, so we do not need to worry about
run-time. In slower implementations, it is recommended to compute the factorization
using a subset of our gene expression data by defining a smaller set of genes
...
Now we can run the NNMF using the `RunNMF()` function. For the NMF implementation
we are using here, we do not need to manually define the number of factors to use
for the factorization. It will run cross-validation to find the best rank (number 
of factors), and it will learn a model at that rank.

_Comment_ : While we do not need to subset our gene expression data here, this can
be achieved with the `SubsetSTData()` function by feeding it a vector of genes to keep
in the `features=` argument. 

_Comment_ : The package responsible for this very-fast NMF implementation is not quite 
stable. In order to run the `RunNMF()` function, we first need to quickly
"hack" our Seurat object. This issue will be addressed by the developers of the 
`RunNMF()` package, but they are not there yet unfortunately. This means we get
to come up with some workarounds, yipee. In this case, we will create a compatible
assay inside of our Seurat object that will contain the expression data of the 
3000 most variable genes for all the spots. **Plz do not change the seed**

```{r run_nnmf}

# first let us hack our seurat object by adding a compatible gene expression assay 
# with only the variable genes
se.HER2[["Spatial3"]] <- as(
  object = subset(se.HER2[["Spatial"]], features = VariableFeatures(se.HER2)),
  Class = "Assay"
)

# see that now we have 2 assays in our seurat object: 
#   - Spatial is our original data
#   - SCT stores our SCTransform() normalized data
#   - Spatial3 is the assay we jus created, compatible with the RunNMF() function
Assays(se.HER2)

# next up, let's run NMF to identify our sweet gene expression factors on the compatible assay
# if we wanted to find a defined number of factors, we need to feed an integer to the -k- variable
# for automatic factor identification, we should set it to NULL
set.seed(1337)
se.HER2 <- RunNMF(se.HER2, 
                  assay = "Spatial3",
                  # k = 10
                  k = NULL
)
...
We can actually take a look at how the method has determined which rank (number of factors)
is optimal for the inputted dataset. 
...
We can see that the global minimum of the bottom function is achieved at x = 20, 
so the model has determined that 20 factors can describe the variability of our dataset.
...
In both matrices, we have **20** columns, 1 for each factor. In the first matrix _H_,
...
```{r plot_nnmf, fig.width=20, fig.height=20}

# grab the total number of factors that have been identified
k <- ncol(H)
# plotting

MapFeatures(se.HER2,
            features = paste0("NMF_", 1:k),
            override_plot_dims = TRUE, image_use = NULL,
            scale = "free",
...
> What are the 10 spots with the highest contribution to factor 1? What are the 10
> genes with the highest contribution to factor 9?
>
...
q10.ans1 <- as.data.frame(H) %>% 
  arrange(desc(NMF_1)) %>% 
  rownames()
q10.ans1 <- q10.ans1[1:10]
q10.ans2 <- as.data.frame(W) %>% 
  arrange(desc(NMF_1)) %>% 
  rownames()
q10.ans2 <- q10.ans2[1:10]

# print answers
cat(sprintf("The spots with the highest contribution to factor 1 are: \n%s", paste(q10.ans1, collapse = "\n")), "\n\n")
cat(sprintf("The genes with the highest contribution to factor 9 are: \n%s", paste(q10.ans2, collapse = "\n")))
...
Factor 20 looks kinda interesting, almost as if it's surrounding the DCIS
...
So what we see here is *a lot* of genes whose names start with *IG*. Is the body
reacting somehow to the cancerous lesion?
...
> you consider most likely that factor 21 is associated with.
>
> **Processes**
>
...
print(sprintf("Factor 20 is associated with process %d", ans.q11))
...
top.2.genes <- as.data.frame(W) %>% 
  rownames_to_column(var = "gene") %>% 
  pivot_longer(!gene, names_to = "factor", values_to = "weight") %>% 
  group_by(factor) %>% 
  slice_max(order_by = weight, n = 2) %>% 
  relocate(weight, .before = factor) %>% 
  mutate(factor = str_replace(factor, "NMF", "factor"))
top.2.genes <- top.2.genes[mixedorder(top.2.genes$factor),]
...
deconvolute our spatial data with them.
...
Fortunately we have some connections at Dinosaur Island, where a group at the Garvan
...
stsc <- t(stsc)
colnames(stsc) <- str_replace(colnames(stsc), "_1", "")
...
our beloved `MapFeatures()` comes in handy again
...
more than 50 cell types (not uncommon), then we'd have to generate _many many plots_. 
Also, with more cell types, it becomes increasingly hard to get a
...
through $50!/(48!2!) = 1225$ pairs... also known as "i really do not want to do that".

In order to condense the rich information that the single cell mapping has given
us, we can instead look at the (Spearman) correlation of the proportion values
...
# compute spearman correlation of cell types across spots
cors <- LayerData(se.HER2) %>%
  as.matrix() %>%
  t() %>% 
  cor(method = "spearman")
...
**Extra information**: Now that you have made it past the lab's exercises, if you 
are more curious about what one can do bioinformatically with this type of data,
feel free to check out `semla`'s website and GitHub. There you can find a lot of 
tutorials, and we are always looking to improve the package! :)
[github](https://github.com/ludvigla/semla)
[github](https://ludvigla.github.io/semla/)
