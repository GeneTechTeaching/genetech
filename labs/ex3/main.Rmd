...
# Print object size
cat(sprintf(
  "Size of sparse matrix: %s\nSize of 'normal' matrix: %s", 
  utils:::format.object_size(object.size(sparse.exprMat), "auto"),
  utils:::format.object_size(object.size(exprMat), "auto")
))
```
...
cat(sprintf("Number of cells after filtering: %s\nNumber of genes after filtering: %s", n.cells.filt, n.genes.filt))
```
...
**NB**: Failed the above check? Try to rerun all chunks above to make sure you have not modified the `se` or `se.subset` objects in some way. Select the dropdown menu next to "Run" and click "Run All Chunks Above".
...
cat(sprintf(
  "Number of clusters detected at a resolution of 0.3: %s\nNumber of clusters detected at a resolution of 1.2: %s", 
  nClusters.0.3,
  nClusters.1.2
))
```
...
> Now run another FEA but this time on T-cell markers and set the lower threshold for `avg_log2FC` to 0.5 when selecting the marker genes. Use the same reference database as before (<GO:BP>). Then report the term name and p-value for the most significant term.
>
